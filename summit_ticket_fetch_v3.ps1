...
$loggedby = "3"
$loggedby_mail = "bharat.peddakota@wipro.com"
...
$filter = "{'OrgID': '$org_ID' ,'Instance': '$Instance' ,'Status': '$State','CallerName': '$loggedby','LoggedBy_EmailID':'$loggedby_mail','PageSize':10}"
        ...
        if ($read_output -notlike "*NotReachable*"){
            ...
            foreach($ticket in $ticket_dump){
                ...
                if ($sub_data -notlike "*NotReachable*"){
                    ...
                    Write-Host "item"$omplete_Data.Configuration_Item_ID
                    Write-Host "id"$id
                    
                    [hashtable]$Object =@{}
                $Object."inc_host" = $short_Desc
                $Object."id" = $id
